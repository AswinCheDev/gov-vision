...
## Progress 1 Leftovers Now Done (verified in codebase)

- AI routes now cover anomaly list + acknowledge; analytics routes include forecast and risk heatmap
- Risk heatmap UI is implemented (Dashboard and Risk page) instead of the Progress 1 placeholder
- Forecast and risk scoring pipelines are wired to the ML service and persisted for UI consumption
- Report generation is fully wired (CSV/Excel/PDF) with builder, history, and schedules in the UI
- Department ID normalization to FI001/HR002/OP003/IT004/CS005 is used in KPI and report flows

---

## What's Left To Do (verified, simple points)

- Re-enable JWT/role enforcement on analytics routes and remove the dev test-role bypass in JWT middleware
- Implement Module 3 pages still missing: Decision Analytics, Compliance Analytics, Department Performance, KPI Config (Admin)
- Add AI endpoints from the Module 3 guide (/api/ai/forecast, /api/ai/risk-scores, /api/ai/retrain) or align the UI to current /analytics endpoints
- Generate forecast targets for pending_workload and sla_misses (UI exposes these but cron job does not produce them)
- Expand report features to spec: widget selection, templates, schedule recipients + email delivery, richer PDF/Excel sections
- Add weekly retraining for Prophet and Random Forest (server retrain job is empty; /ml/models/train covers Isolation Forest only)
- Integrate Module 2 risk/compliance features into risk scoring (current job uses decision-only features)
- Align compliance trend filters (client sends deptId, server expects deptIds) and verify Redis cache-hit evidence at runtime
